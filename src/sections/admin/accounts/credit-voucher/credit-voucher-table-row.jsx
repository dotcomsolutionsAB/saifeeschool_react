...
const CreditVoucherTableRow = ({ row, index, refetch }) => {
  const { logout } = useAuth();
  const [anchorEl, setAnchorEl] = useState(null);
  const [creditVoucherEditModalOpen, setCreditVoucherEditModalOpen] =
    useState(false);
  ...
  // credit voucher modal handler

  const handleCreditEditModalOpen = () => {
    setCreditVoucherEditModalOpen(true);
  };

  const handleCreditEditModalClose = () => {
    setCreditVoucherEditModalOpen(false);
    handleMenuClose();
  };

  const handlePrint = async () => {
    setIsPrintLoading(true);
    const response = await printCreditPdf(row);
  ...
  const handleDelete = async () => {
    ...
    const response = await deleteCreditVoucher(row);
  ...
  return (
    <>
      <TableRow hover tabIndex={-1}>
        ...
        <TableCell>{row?.credit_no || "-"}</TableCell>
      ...
      >
        ...
        <MenuItem
          onClick={handleCreditEditModalOpen}
          sx={{ color: "primary.main" }}
        >
      ...
      {/* Create Edit Credit Voucher Dialog */}
      <CreateEditCreditVoucherModal
        open={creditVoucherEditModalOpen}
        onClose={handleCreditEditModalClose}
        refetch={refetch}
        detail={row}
      />
    </>
...
CreditVoucherTableRow.propTypes = {
  row: PropTypes.object,
  refetch: PropTypes.func,
  index: PropTypes.number,
};

export default CreditVoucherTableRow;
